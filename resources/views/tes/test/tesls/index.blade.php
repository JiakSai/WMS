...
<script>
    ...
    $(document).ready(function() {
        ...
        templateDataTable = $('#templateDataTable').DataTable({
                    ...
                    buttons: [{
                        ...
                        'spacer', 'spacer',
                        {
                            ...
                            action: function(e, dt, node, config) {
                                $('#modal').modal('show');
                                modal('{{ route('tes.test.tesls.create', ['organisation' => $organisation->id]) }}',2);
                            }
                        }
                    ]
                },
            ...
            ajax: {
                method: "POST",
                url: '{{ route('tes.test.tesls.show', ['organisation' => $organisation->id]) }}',
                data: function(d) {
                    d._token = '{{ csrf_token() }}';
            ...
            columns: [{
                ...
                },
                {
                    ...
                    mRender: function(data, type, row) {
                        ...
                        +'<a href="#" class="dropdown-item" data-bs-toggle="modal" data-bs-target="#modal" onclick="modal(\'' + '{{ route('tes.test.tesls.edit', ['organisation' => $organisation->id]) }}'+ '\', \'' + id + '\')"><i class="bi bi-pencil-square"></i> Edit</a>'  
    ...
    function btnRemove(id) {
        $.ajax({
            type: "DELETE",
            url: '{{ route('tes.test.tesls.destroy', ['organisation' => $organisation->id]) }}',
            dataType: 'json',
            data: { 
                id: id,
