...
<div class="hljs-container rounded-bottom">
    <!-- html -->
    <table id="itemDataTable" class="table table-bordered table-xs w-100 fw-semibold text-nowrap mb-3">
        <thead>
            <tr>
                <th>ID</th>
                <th>Name</th>
...
<script>
    ...
    $(document).ready(function() {
        ...
        itemDataTable = $('#itemDataTable').DataTable({
                        ...
                        @if($hasCreatePermission)
                            {
                                ...
                                action: function(e, dt, node, config) {
                                    $('#modal').modal('show');
                                    modal('{{ route('wms.item.datas.create', ['organisation' => $organisation->id]) }}',2);
                                }
                            }
                        @endif
                    ]
            ...
            ajax: {
                method: "POST",
                url: '{{ route('wms.item.datas.show', ['organisation' => $organisation->id]) }}',
                data: function(d) {
                    d._token = '{{ csrf_token() }}';
                    d.name = itemDataTable_data.name;
                }
            },
            language: {
            ...
            columns: [{
                ...
                },
                {
                    ...
                    mRender: function(data, type, row) {
                        ...
                        +'<a href="#" class="dropdown-item" data-bs-toggle="modal" data-bs-target="#modal" onclick="modal(\'' + '{{ route('wms.item.datas.edit', ['organisation' => $organisation->id]) }}'+ '\', \'' + id + '\')"><i class="bi bi-pencil-square"></i> Edit</a>'  
    ...
    function btnRemove(id) {
        $.ajax({
            type: "DELETE",
            url: '{{ route('wms.item.datas.destroy', ['organisation' => $organisation->id]) }}',
            dataType: 'json',
            data: { 
                id: id,
    ...
    $("#itemSearchForm").submit(function(event) {
        ...
        itemDataTable_data.name = $(this).find('[name="name"]').val();
