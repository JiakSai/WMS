<div class="modal-header">
    <h1 class="modal-title fs-5" id="exampleModalLabel">Add Item Group Child</h1>
...
<form id="addForm">
    <div class="modal-body" style="max-height: calc(100vh - 200px);overflow-y: auto;">
        <div class="mb-3 row" id="formContent">
            <div class="row mb-3">
                <label class="col-sm-3 col-form-label">Item Group Child Name</label>
                <div class="col-sm-9">
                    <input type="text" class="form-control" name="name">
                    <input type="hidden" class="form-control" name="parentId" value="{{ $parentGroup->id }}"> 
                </div>
            </div>
        </div>
    </div>
...
<script>
    $("#addForm").submit(function(event) {
        ...
        $.ajax({
            method: "POST",
            url: '{{ route('wms.item.grpcs.store-child', ['organisation' => $organisation->id]) }}',
            data: formData,
            success: function(d) {
                // Hide the spinner
