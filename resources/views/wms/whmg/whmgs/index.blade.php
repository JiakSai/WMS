    ...
    <div class="d-flex">
        <div class="ms-auto mb-3">
            ...
            <div class="input-group flex-nowrap">
                <input type="text" class="form-control text-uppercase" name="name" placeholder="Warehouse Name" autofocus />
...
<div class="hljs-container rounded-bottom">
    <!-- html -->
    <table id="warehouseDataTable" class="table table-bordered table-xs w-100 fw-semibold text-nowrap mb-3">
        <thead>
            <tr>
                <th>ID</th>
                <th>Name</th>
...
<script>
    ...
    $(document).ready(function() {
        ...
        warehouseDataTable = $('#warehouseDataTable').DataTable({
            layout: {
                topStart: {
                    buttons: [{
                            extend: 'pageLength',
                    ...
                    buttons: [{
                        ...
                        'spacer', 'spacer',
                        {
                            ...
                            action: function(e, dt, node, config) {
                                $('#modal').modal('show');
                                modal('{{ route('wms.whmg.whmgs.create', ['organisation' => $organisation->id]) }}',2);
                            }
                        }
                    ]
                },
            ...
            ajax: {
                method: "POST",
                url: '{{ route('wms.whmg.whmgs.show', ['organisation' => $organisation->id]) }}',
                data: function(d) {
                    d._token = '{{ csrf_token() }}';
                    d.name = warehouseDataTable_data.name;
                }
            },
            language: {
            ...
            columns: [{
                ...
                },
                {
                    ...
                    mRender: function(data, type, row) {
                        ...
                        +'<a href="#" class="dropdown-item" data-bs-toggle="modal" data-bs-target="#modal" onclick="modal(\'' + '{{ route('wms.whmg.whmgs.edit', ['organisation' => $organisation->id]) }}'+ '\', \'' + id + '\')"><i class="bi bi-pencil-square"></i> Edit</a>'  
    ...
    function btnRemove(id) {
        $.ajax({
            type: "DELETE",
            url: '{{ route('wms.whmg.whmgs.destroy', ['organisation' => $organisation->id]) }}',
            dataType: 'json',
            data: { 
                id: id,
    ...
    $("#warehouseSearchForm").submit(function(event) {
        ...
        warehouseDataTable_data.name = $(this).find('[name="name"]').val();
