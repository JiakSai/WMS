...
<script>
    ...
    $(document).ready(function() {
        ...
        assignDataTable = $('#assignDataTable').DataTable({
                    ...
                    buttons: [{
                        ...
                        'spacer', 'spacer',
                        {
                            ...
                            action: function(e, dt, node, config) {
                                $('#modal').modal('show');
                                modal('{{ route('wms.role.asgns.create', ['organisation' => $organisation->id]) }}', 2)
                            }
                        }
                    ]
                },
            ...
            "ajax": {
                method: "POST",
                url: '{{ route('wms.role.asgns.show', ['organisation' => $organisation->id]) }}',
                data: function(d) {
                    d._token = '{{ csrf_token() }}';
            ...
            'columns': [
                ...
                { 'data' : 'updated_by'},
                {
                    mRender: function (data, type, row) {
                        ...
                        +'<a href="#" class="dropdown-item"  data-bs-toggle="modal" data-bs-target="#modal" onclick="modal(\'' + '{{ route('wms.role.asgns.edit', ['organisation' => $organisation->id]) }}' + '\', \'' + id + '\')"><i class="bi bi-person-fill-add"></i> Assign User</a>'    
                        +'<a href="#" class="dropdown-item"  data-bs-toggle="modal" data-bs-target="#modal" onclick="modal(\'' + '{{ route('wms.role.asgns.edit', ['organisation' => $organisation->id]) }}' + '\', \'' + id + '\')"><i class="bi bi-pencil-square"></i> Edit</a>'    
    ...
    function btnRemove(id) {
        $.ajax({
            type: "POST",
            url: '{{ route('wms.role.asgns.destroy', ['organisation' => $organisation->id]) }}',
            dataType: 'json',
            data: {         
                id: id,
