        ...
        <div class="ms-auto mb-3">
            <div class="input-group flex-nowrap">
                ...
                <div class="me-2">
                    ...
                    <input type="datetime-local" id="from_date" class="form-control" name="from_date" placeholder="From Date" />
                </div>
                <div class="me-2">
                    <label for="to_date" class="form-label">To Date</label>
                    <input type="datetime-local" id="to_date" class="form-control" name="to_date" placeholder="To Date" />
                ...
                {{-- <input type="hidden" id="from_date_hidden" name="from_date_hidden" />
                <input type="hidden" id="to_date_hidden" name="to_date_hidden" /> --}}
            </div>
        </div>
    </div>
</form>
...
<div class="hljs-container rounded-bottom">
    <!-- html -->
    <table id="InvoiceDataTable" class="table table-bordered table-xs w-100 fw-semibold text-nowrap mb-3">
        <thead>
            <tr>
                <th>DocumentType</th>
                <th>InvoiceID</th>
...
<script>
    ...
    $('#to_date').val(toDate);
        ...
        InvoiceDataTable = $('#InvoiceDataTable').DataTable({
                    ...
                    buttons: [{
                        ...
                        'spacer', 'spacer',
                        {
                            ...
                            action: function(e, dt, node, config) {
                                $('#modal').modal('show');
                                modal('{{ route('env.sinv.sinvs.create', ['organisation' => $organisation->id]) }}',2);
                            }
                        }
            ...
            ajax: {
                method: "POST",
                url: '{{ route('env.sinv.sinvs.show', ['organisation' => $organisation->id]) }}',
                data: function(d) {
                    d._token = '{{ csrf_token() }}';
                    d.name = InvoiceDataTable_data.name;
    ...
    InvoiceDataTable_data.from_date = $(this).find('[name="from_date"]').val();
    InvoiceDataTable_data.to_date = $(this).find('[name="to_date"]').val();

    // // Validate dates
    if (InvoiceDataTable_data.from_date && InvoiceDataTable_data.to_date) {
        const fromDate = new Date(InvoiceDataTable_data.from_date);
        const toDate = new Date(InvoiceDataTable_data.to_date);
