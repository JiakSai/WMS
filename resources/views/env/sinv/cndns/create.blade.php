...
<script>
    ...
    let table = $('#CNDNDataTable').DataTable();
...
function fetchTemplateCSV() {
    return new Promise((resolve, reject) => {
        fetch('/Template/CNDN_Template.csv')
...
function downloadCSV(csvData, submitButton, spinner) {
    ...
    $.ajax({
        method: "POST",
        // url: "env.sinv.cndns.save-csv"
        url: '{{ route('env.sinv.cndns.save-ftp', ['organisation' => $organisation->id]) }}',
        data: { 
            _token: '{{ csrf_token() }}',
            csvData: csvData 
        },
