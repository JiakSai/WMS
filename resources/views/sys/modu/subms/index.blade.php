    ...
    <div class="d-flex">
        <div class="ms-auto mb-3">
            ...
            <div class="input-group flex-nowrap">
                <input type="text" class="form-control text-uppercase" name="name" placeholder="Sub Module Name" autofocus />
...
<div class="hljs-container rounded-bottom">
    <!-- html -->
    <table id="subDataTable" class="table table-bordered table-xs w-100 fw-semibold text-nowrap mb-3">
        <thead>
            <tr>
                <th>ID</th>
                <th>Name</th>
...
<script>
    ...
    $(document).ready(function() {
        ...
        subDataTable = $('#subDataTable').DataTable({
            layout: {
                topStart: {
                    buttons: [{
                            extend: 'pageLength',
                    ...
                    buttons: [{
                        ...
                        'spacer', 'spacer',
                        {
                            ...
                            action: function(e, dt, node, config) {
                                $('#modal').modal('show');
                                modal('{{ route('sys.modu.subms.create', ['organisation' => $organisation->id]) }}',
                                    2);
                            }
                        }
                    ]
            ...
            ajax: {
                method: "POST",
                url: '{{ route('sys.modu.subms.show', ['organisation' => $organisation->id]) }}',
                data: function(d) {
                    d._token = '{{ csrf_token() }}';
                    d.name = subDataTable_data.name;
                }
            },
            language: {
    ...
    function btnRemove(id) {
        $.ajax({
            type: "DELETE",
            url: '{{route('sys.modu.subms.destroy', ['organisation' => $organisation->id])}}',
            dataType: 'json',
            data: { 
                id: id,
    ...
    function btnActivate(id){
        $.ajax({
            method: "POST",
            url : '{{route('sys.modu.subms.activate', ['organisation' => $organisation->id])}}',
            dataType: 'json',
            data:{
                id: id,
                _token: '{{ csrf_token() }}'
    ...
    function btnDeactivate(id){
        $.ajax({
            method: "POST",
            url : '{{route('sys.modu.subms.deactivate', ['organisation' => $organisation->id])}}',
            dataType: 'json',
            data:{
                id: id,
                _token: '{{ csrf_token() }}'
    ...
    $("#subSearchForm").submit(function(event) {
        ...
        subDataTable_data.name = $(this).find('[name="name"]').val();
