...
<div class="hljs-container rounded-bottom">
    ...
    <table id="authDataTable"  class="table table-bordered table-xs w-100 fw-semibold text-nowrap mb-3">
        <thead>
            <tr>
                <th>ID</th>
                <th>Permissions</th>
...
<script>
    ...
    $(document).ready(function() {
        ...
        authDataTable = $('#authDataTable').DataTable({
                    ...
                    buttons: [{
                        ...
                        'spacer', 'spacer',
                        {
                            ...
                            action: function(e, dt, node, config) {
                                $('#modal').modal('show');
                                modal('{{ route('sys.role.auths.create', ['organisation' => $organisation->id]) }}', 2)
                            }
                        }
                    ]
                },
            ...
            "ajax": {
                method: "POST",
                url: '{{ route('sys.role.auths.show', ['organisation' => $organisation->id]) }}',
                data: function(d) {
                    d._token = '{{ csrf_token() }}';
                    d.name = authDataTable_data.searchGroup;
                }
            },
            ...
            'columns': [
                ...
                { 'data' : 'updated_by'},
                {
                    mRender: function (data, type, row) {
                        ...
                        +'<a href="#" class="dropdown-item"  data-bs-toggle="modal" data-bs-target="#modal" onclick="modal(\'' + '{{ route('sys.role.auths.edit', ['organisation' => $organisation->id]) }}' + '\', \'' + id + '\')"><i class="bi bi-pencil-square"></i> Edit</a>'    
        ...
        document.querySelectorAll('a.toggle-vis').forEach((el) => {
            el.addEventListener('click', function (e) {
                ...
                let column = authDataTable.column(columnIdx);
    ...
    function btnRemove(id) {
        $.ajax({
            type: "DELETE",
            url: '{{ route('sys.role.auths.destroy', ['organisation' => $organisation->id]) }}',
            dataType: 'json',
            data: {         
                id: id,
    ...
    $("#authSearchForm").submit(function(event) {
        event.preventDefault();
        
        // Serialize the form data
        let searchFormData = $(this).serialize();

        authDataTable_data.searchGroup = $(this).find('[name="name"]').val();
