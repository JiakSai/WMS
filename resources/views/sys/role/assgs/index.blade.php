...
<script>
    ...
    $(document).ready(function() {
        ...
        assignDataTable = $('#assignDataTable').DataTable({
                    ...
                    buttons: [{
                        ...
                        'spacer', 'spacer',
                        {
                            ...
                            action: function(e, dt, node, config) {
                                $('#modal').modal('show');
                                modal('{{ route('sys.role.assgs.create', ['organisation' => $organisation->id]) }}', 2)
                            }
                        }
                    ]
                },
            ...
            "ajax": {
                method: "POST",
                url: '{{ route('sys.role.assgs.show', ['organisation' => $organisation->id]) }}',
                data: function(d) {
                    d._token = '{{ csrf_token() }}';
            ...
            'columns': [
                ...
                { 'data' : 'updated_by'},
                {
                    mRender: function (data, type, row) {
                        ...
                        +'<a href="#" class="dropdown-item"  data-bs-toggle="modal" data-bs-target="#modal" onclick="modal(\'' + '{{ route('sys.role.assgs.edit', ['organisation' => $organisation->id]) }}' + '\', \'' + id + '\')"><i class="bi bi-pencil-square"></i> Edit</a>'    
    ...
    function btnRemove(id) {
        $.ajax({
            type: "POST",
            url: '{{ route('sys.role.assgs.destroy', ['organisation' => $organisation->id]) }}',
            dataType: 'json',
            data: {         
                id: id,
