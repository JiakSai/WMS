    ...
    <div class="modal-body" style="max-height: calc(100vh - 200px);overflow-y: auto;">
        <div class="mb-2 row">
            ...
            <div class="col-md-5 mb-2">
                ...
                <select class="form-select" name="group" data-placeholder="Select Warehouse">
        ...
        <div class="mb-2 row">
            <div class="col-md-12 mb-2">
                <table id="myTable" class="table table-bordered text-center order-list">
                    ...
                    <tbody>
                        @foreach($selectedWarehouses as $index => $selectedWarehouse)
                        <tr>
                            ...
                            <td class="col-md-7">
                                <select class="form-select warehouse-select" name="warehouse[]">
                                    ...
                                    @foreach($warehouses as $w)
                                        <option value="{{ $w->id }}" 
                                            @if($w->id == $selectedWarehouse) selected @endif>
                                            {{ $w->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <td class="col-md-1" style="padding-top: 13px">
                                <input type="radio" class="form-check-input default-warehouse" name="default_warehouse" value="{{ $index }}" 
                                    @if($defaultWarehouse && $defaultWarehouse->id == $selectedWarehouse) checked @endif>
                            </td>
                        ...
                        @endforeach
                        <tr id="addRow">
                            <td class="col-md-1" style="padding-top: 15px">{{ count($selectedWarehouses) + 1 }}</td>
                            <td class="col-md-7">
                                <select class="form-select warehouse-select" name="warehouse[]">
                            ...
                            <td class="col-md-1" style="padding-top: 13px">
                                <input type="radio" class="form-check-input default-warehouse" name="default_warehouse" value="" disabled>
                            </td>
                            <td class="col-md-1" style="padding-top: 11px">
                                <a href="#" class="btn btn-dark btn-sm me-1 deleteRow"><i class="bi bi-trash"></i></a>
                            </td>
...
$(document).ready(function () {
    ...
    function updateRowNumbers() {
        $("table.order-list tbody tr").each(function(index) {
            ...
            $(this).find("input[type='radio'].default-warehouse").val(index);
        });
    }
    ...
    $("#addForm").submit(function(event) {
        ...
        $.ajax({
            method: "POST",
            url: '{{ route('editUserSubmit', ['warehouse' => $warehouse->id]) }}',
            data: formData,
            success: function(d) {
                // Hide the spinner
