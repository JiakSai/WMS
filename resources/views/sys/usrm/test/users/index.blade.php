{{-- <div class="d-flex align-items-center mb-3">
    <div class="ms-auto">
        ...
        onclick="modal('{{route('addUser', ['warehouse' => $warehouse->id])}}', 2)"><i class="fa fa-plus-circle me-1"></i> Add User</a> 
    </div>
</div> --}}
...
<script>
    ...
    $(document).ready(function() {
        ...
        userDatatable = $('#userDataTable').DataTable({
                    ...
                    buttons: [{
                        ...
                        'spacer', 'spacer',
                        {
                            ...
                            action: function(e, dt, node, config) {
                                $('#modal').modal('show');
                                modal('{{ route('addUser', ['warehouse' => $warehouse->id]) }}',
                                    2);
                            }
                        }
                    ]
            ...
            ajax: {
                method: "POST",
                url: '{{ route('getData', ['warehouse' => $warehouse->id]) }}',
                data: function(d) {
                    d._token = '{{ csrf_token() }}';
            ...
            columns: [{
...

                {
                    ...
                    mRender: function(data, type, row) {
                        ...
                        +'<li><a href="#" class="dropdown-item" data-bs-toggle="modal" data-bs-target="#modal" onclick="modal(\'' + '{{ route('editUser', ['warehouse' => $warehouse->id]) }}' + '\', \'' + username + '\')"><i class="bi bi-pencil-square"></i> Edit</a></li>'   
    ...
    function btnResetPassword(id) {
        $.ajax({
            method: "POST",
            url: '{{ route('resetDefaultPassword', ['warehouse' => $warehouse->id]) }}',
            dataType: 'json',
            data: {
                username: id,
    ...
    function btnActivate(id) {
        $.ajax({
            method: "POST",
            url: '{{ route('activateUser', ['warehouse' => $warehouse->id]) }}',
            dataType: 'json',
            data: {
                username: id,
                _token: '{{ csrf_token() }}'
    ...
    function btnDeactivate(id) {
        $.ajax({
            method: "POST",
            url: '{{ route('deactivateUser', ['warehouse' => $warehouse->id]) }}',
            dataType: 'json',
            data: {
                username: id,
                _token: '{{ csrf_token() }}'
